...
import { getMixedOilProducts, saveMixedOilProducts, getAllProducts } from '../../../constants/productData';
...
const MixedOilAdmin = ({ shopName: _shopName, villageName: _villageName, theme, cart, updateQuantity, onBack, onReviewOrder }: Props) => {
    const isDark = theme === 'dark';
    const { toasts, showToast, removeToast } = useToast();

    const [products, setProducts] = useState<Product[]>(getMixedOilProducts());
    const [showProductModal, setShowProductModal] = useState(false);
    const [editingProduct, setEditingProduct] = useState<Product | null>(null);
    const [productForm, setProductForm] = useState({ name: '', brand: 'Varsini', size: '', price: '', unit: 'BOX', weight: '' });
    ...
    const openAddProduct = () => {
        ...
        setProductForm({ name: '', brand: 'Varsini', size: '', price: '', unit: 'BOX', weight: '' });
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-10">
                <div>
                    ...
                    <p className="text-sm font-bold text-slate-500 mt-2 uppercase tracking-widest flex items-center gap-2">
                        CATEGORY: <span className="text-blue-500 font-black">Mixed Oil</span>
                    ...
                    >
                        ...
                        <div className="flex gap-8 items-center">
                            <div className={`w-28 h-28 rounded-[32px] flex items-center justify-center text-4xl transition-transform group-hover:scale-110 group-hover:rotate-3
                                ${isDark ? 'bg-slate-800 border border-white/5' : 'bg-blue-50 border border-blue-100 shadow-inner'}`}>
                                📦
                            </div>
                                ...
                                <div className="flex items-center justify-between mt-auto">
                                    ...
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div className={`flex items-center gap-5 p-2 rounded-3xl border ${isDark ? 'bg-slate-950/50 border-white/5' : 'bg-slate-50/50 border-slate-100 shadow-inner'}`}>
                                            ...
                                            <span className={`text-2xl font-black w-8 text-center ${isDark ? 'text-white' : 'text-slate-900'}`}>{cart[product.id] || 0}</span>
            ...
            {totalItems > 0 && (
                <div className="fixed bottom-4 left-4 right-4 lg:bottom-10 lg:left-[calc(18rem+2.5rem)] lg:right-10 z-[100] animate-in slide-in-from-bottom-10 duration-700">
                    <div className={`max-w-6xl mx-auto rounded-[30px] lg:rounded-[50px] p-4 md:p-6 lg:p-8 border backdrop-blur-3xl shadow-2xl transition-all
                        ${isDark ? 'bg-slate-900/90 border-white/10 shadow-blue-500/10' : 'bg-white/90 border-slate-200 shadow-slate-300'}`}>
                    ...
                    <div className={`relative rounded-3xl w-full max-w-md shadow-2xl border p-8 ${isDark ? 'bg-slate-900 border-white/10' : 'bg-white border-slate-100'}`}>
                        ...
                        <div className="space-y-4">
                            <div>
                                ...
                                <input className={inputCls} value={productForm.name} onChange={e => setProductForm(f => ({ ...f, name: e.target.value }))} placeholder="e.g. Mixed Oil" />
                            </div>
                            <div className="grid grid-cols-2 gap-4">
                                ...
                                <div>
                                    ...
                                    <input className={inputCls} value={productForm.size} onChange={e => setProductForm(f => ({ ...f, size: e.target.value }))} placeholder="e.g. 1 L box" />
                                </div>
                            </div>
                            <div className="grid grid-cols-2 gap-4">
                                <div>

                ...
                <div className="p-12">
                    ...
                    <form onSubmit={onSubmit} className="space-y-8">
                        <div className="space-y-4">
                            ...
                            <div className="space-y-3">
                                <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-4">Node ID</label>
                                <input required type="text" placeholder="Enter Node Prefix (e.g. SAT-01)" className={`w-full rounded-[25px] px-8 py-5 border transition-all font-black text-sm focus:outline-none focus:ring-4 ${theme === 'dark' ? 'bg-slate-800 border-white/10 text-white focus:ring-emerald-500/20 focus:border-emerald-500' : 'bg-slate-50 border-slate-100 text-slate-900 focus:ring-emerald-600/10 focus:border-emerald-600'}`} value={olFormData.node_id} onChange={e => setOlFormData({ ...olFormData, node_id: e.target.value.toUpperCase() })} />
                            </div>
                        </div>

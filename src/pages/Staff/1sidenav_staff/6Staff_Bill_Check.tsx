...
const StaffBillCheck = ({ theme, userProfileName }: Props) => {
    ...
    const loadUnverifiedBills = async () => {
        try {
            ...
            const mapped = res.data
                .filter((b: any) => (b.created_by || b.createdBy) === userProfileName)
                .map((b: any) => ({
                    id: b.id,
        ...
        <div className={`space-y-8 animate-in fade-in slide-in-from-right-5 duration-500 ${isDark ? 'text-white' : 'text-slate-900'}`}>
            <div className="flex items-center justify-between gap-6 flex-wrap">
                <div>
                    ...
                    <p className="text-sm font-bold text-slate-400 mt-1">Orders awaiting Admin/Manager verification</p>
                </div>
                <div className={`px-4 py-2 rounded-2xl text-xs font-black uppercase tracking-widest border
                    ...
                    {unverifiedBills.length} In Queue
                </div>
            </div>
            ...
            {unverifiedBills.length === 0 ? (
                <div className={`py-20 text-center rounded-[40px] border ${isDark ? 'bg-slate-900 border-white/5' : 'bg-white border-slate-100 shadow-xl'}`}>
                    ...
                    <p className={`font-black text-xl italic ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>No bills in queue</p>
                    <p className="text-slate-500 mt-2 text-sm font-bold">Your submitted orders will appear here until verified.</p>
                </div>
            ) : (
                <div className="space-y-4">
                    {unverifiedBills.sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()).map(bill => (
                        <div key={bill.id} className={`p-6 rounded-[30px] border transition-all ${isDark ? 'bg-slate-900 border-amber-500/20 shadow-lg shadow-amber-500/5' : 'bg-amber-50/50 border-amber-200 shadow-xl shadow-amber-500/10'}`}>
                            ...
                            <div className="grid grid-cols-12 gap-6 items-center">
                                ...
                                <div className="col-span-12 md:col-span-3">
                                    ...
                                    <p className={`text-sm font-black italic tracking-tighter ${isDark ? 'text-amber-400' : 'text-amber-600'}`}>
                            ...
                            <div className="mt-6 pt-6 border-t border-dashed flex items-center justify-end gap-3 border-amber-200 dark:border-white/10">
                                <button
            ...
            {editingBill && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                    ...
                    <div className={`relative rounded-[40px] w-full max-w-4xl max-h-[90vh] flex flex-col shadow-2xl animate-in zoom-in-95 duration-500 border ${isDark ? 'bg-slate-900 border-amber-500/20' : 'bg-white border-slate-100'}`}>
                        {/* Header */}
                        ...
                        {/* Filters */}
                        <div className={`px-8 pb-4 flex gap-4 border-b shrink-0 ${isDark ? 'border-amber-500/20' : 'border-slate-100'}`}>
                            <div className="relative flex-1">
                                ...
                                <input
                                    ...
                                    className={`w-full rounded-[20px] pl-10 pr-5 py-3 text-sm font-bold border outline-none transition-all
                                        ${isDark ? 'bg-slate-800/50 border-white/10 text-white placeholder:text-slate-500 focus:border-amber-500' : 'bg-slate-50 border-slate-200 text-slate-900 placeholder:text-slate-400 focus:border-blue-500 hover:border-slate-300'}`}
                                />
                            </div>
                            <select
                                ...
                                className={`rounded-[20px] px-5 py-3 text-sm font-bold border outline-none cursor-pointer transition-all
                                    ${isDark ? 'bg-slate-800/50 border-white/10 text-white focus:border-amber-500' : 'bg-slate-50 border-slate-200 text-slate-900 focus:border-blue-500 hover:border-slate-300'}`}
                            >
                                        ...
                                        return (
                                            <div key={p.id} className={`p-4 rounded-[24px] border flex flex-col justify-between transition-all ${isDark ? 'bg-slate-800/30 border-white/5' : 'bg-white border-slate-100 shadow-sm'} ${qty > 0 ? (isDark ? 'border-amber-500/50 bg-amber-500/5' : 'border-blue-500/50 shadow-md bg-blue-50/50') : ''}`}>
                                                <div className="flex items-start justify-between mb-3">
                                                    ...
                                                    <div className="flex items-center gap-1.5 shrink-0">
                                                        ...
                                                        <input
                                                            ...
                                                            className={`w-16 px-2 py-1.5 rounded-xl border text-xs font-black outline-none text-center transition-all focus:border-amber-500
                                                ...
                                                <div className="flex items-center justify-between mt-auto pt-3 border-t border-dashed border-slate-200 dark:border-white/5">
                                                    <div className={`font-black text-sm ${isDark ? 'text-amber-400' : 'text-blue-600'}`}>
                        ...
                        {/* Footer Totals */}
                        <div className={`p-8 border-t shrink-0 flex items-center justify-between rounded-b-[40px] ${isDark ? 'bg-slate-900/90 border-amber-500/20 backdrop-blur-md' : 'bg-slate-50/90 border-slate-200 backdrop-blur-md'}`}>
                            <div>

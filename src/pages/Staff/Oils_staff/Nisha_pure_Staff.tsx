...
import { getNishaProducts, getCartItems, type Product } from '../../../constants/productData';

// No local CartItem interface needed here as it's handled via shared types
...
const NishaPureStaff = ({ shopName, villageName, theme, cart, updateQuantity, filterSubCategory, onBack, onReviewOrder }: Props) => {
    const isDark = theme === 'dark';
    const { toasts, removeToast } = useToast();

    const allNishaProducts = getNishaProducts();
    const products = allNishaProducts.filter((p: Product) => !filterSubCategory || p.name === filterSubCategory);
        ...
        <div className={`min-h-screen pb-40 animate-in fade-in slide-in-from-right-5 duration-500 ${isDark ? 'text-white' : 'text-slate-900'}`}>
            ...
            <div className="flex items-center gap-6 mb-12">
                ...
                <div>
                    ...
                    <p className="text-sm font-bold text-slate-500 mt-2 uppercase tracking-widest flex items-center gap-2">
                        CATEGORY: <span className="text-emerald-500 font-black">NISHA (Pure Oils)</span>
                    ...
                    >
                        ...
                        <div className="flex gap-8 items-center">
                            ...
                            <div className={`w-28 h-28 rounded-[32px] flex items-center justify-center text-4xl transition-transform group-hover:scale-110 group-hover:rotate-3
                                ${isDark ? 'bg-slate-800 border border-white/5' : 'bg-emerald-50 border border-emerald-100'}`}>
                                {product.icon || '🧴'}
                            </div>
                                        ...
                                        <div className={`flex flex-wrap sm:flex-nowrap items-center gap-4 sm:gap-6 p-4 rounded-3xl border ${isDark ? 'bg-slate-900/50 border-white/5' : 'bg-slate-50 border-slate-200 shadow-inner'}`}>
                                            ...
                                            <div className="flex flex-col items-center flex-1 sm:flex-none">
                                                ...
                                                <div className={`flex items-center gap-1 mt-1 p-1 rounded-2xl border ${isDark ? 'bg-slate-950 border-white/10' : 'bg-white border-slate-200 shadow-sm'}`}>
                                                    ...
                                                    />
                                                    <button onClick={() => updateQuantity(product.id + '_box', 1)} className="w-8 h-8 rounded-xl bg-emerald-500 text-white flex items-center justify-center transition-all hover:bg-emerald-600 shadow-md shadow-emerald-500/20">
                                            ...
                                            <div className="flex flex-col items-center flex-1 sm:flex-none">
                                                ...
                                                <div className={`flex items-center gap-1 mt-1 p-1 rounded-2xl border ${isDark ? 'bg-slate-950 border-white/10' : 'bg-white border-slate-200 shadow-sm'}`}>
                                                    ...
                                                    <input
                                                        ...
                                                        className={`w-10 text-center text-[16px] font-black bg-transparent outline-none ${isDark ? 'text-white' : 'text-slate-900'} [-moz-appearance:_textfield][&::-webkit-inner-spin-button]:m-0[&::-webkit-inner-spin-button]:appearance-none`}
                                                    />
                                                    <button onClick={() => updateQuantity(product.id + '_ltr', 0.5)} className="w-8 h-8 rounded-xl bg-emerald-500 text-white flex items-center justify-center transition-all hover:bg-emerald-600 shadow-md shadow-emerald-500/20">
                                                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M12 4v16m8-8H4" /></svg>
                                                    </button>
                                                </div>
                                                <span className="text-sm font-black text-slate-500 mt-2">{product.price * 2}/litre</span>
                                                <span className="text-[10px] font-bold text-emerald-500/60 mt-1">adding 0.5 litre, 1 litre...</span>
                                            </div>
                                            ...
                                            <div className="flex flex-col items-center flex-1 sm:flex-none">
                                                ...
                                                <div className={`flex items-center gap-1 mt-1 p-1 rounded-2xl border ${isDark ? 'bg-slate-950 border-white/10' : 'bg-white border-slate-200 shadow-sm'}`}>
                                                    ...
                                                    <input
                                                        ...
                                                        className={`w-10 text-center text-[16px] font-black bg-transparent outline-none ${isDark ? 'text-white' : 'text-slate-900'} [-moz-appearance:_textfield][&::-webkit-inner-spin-button]:m-0[&::-webkit-inner-spin-button]:appearance-none`}
                                                    />
                                                    <button onClick={() => updateQuantity(product.id, 1)} className="w-8 h-8 rounded-xl bg-emerald-500 text-white flex items-center justify-center transition-all hover:bg-emerald-600 shadow-md shadow-emerald-500/20">
                                                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M12 4v16m8-8H4" /></svg>
                                                    </button>
                                                </div>
                                                <span className="text-sm font-black text-slate-500 mt-2">{product.price}/litre</span>
                                                <span className="text-[10px] font-bold text-emerald-500/60 mt-1">adding 1 litre, 2 litre...</span>
                                            </div>
                                    ...
                                    <div className="flex flex-wrap lg:flex-nowrap items-center justify-between mt-auto gap-4 w-full">
                                        ...
                                        <div className={`flex flex-wrap sm:flex-nowrap items-center gap-4 sm:gap-6 p-4 rounded-3xl border ${isDark ? 'bg-slate-900/50 border-white/5' : 'bg-slate-50 border-slate-200 shadow-inner'}`}>
                                            ...
                                            <div className="flex flex-col items-center flex-1 sm:flex-none">
                                                ...
                                                <span className="text-[10px] font-bold text-emerald-500/60 mt-1">adding 2 litre, 4 litre...</span>
                                            </div>
                                        </div>
                                    </div>
                                ) : (
            ...
            {totalItems > 0 && (
                <div className="fixed bottom-4 left-4 right-4 lg:bottom-10 lg:left-[calc(18rem+2.5rem)] lg:right-10 z-[100] animate-in slide-in-from-bottom-10 duration-700">
                    <div className={`mx-auto rounded-[30px] lg:rounded-[50px] p-4 md:p-6 lg:p-8 border backdrop-blur-3xl shadow-[0_-10px_40px_-15px_rgba(0,0,0,0.1)] lg:shadow-2xl transition-all
                        ${isDark ? 'bg-slate-900/95 border-white/10 shadow-emerald-500/10' : 'bg-white/95 border-slate-200 shadow-slate-300'}`}>

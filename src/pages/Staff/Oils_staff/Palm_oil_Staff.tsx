...
import { getPalmOilProducts, getAllProducts } from '../../../constants/productData';
...
const PalmOilStaff = ({ shopName: _shopName, villageName: _villageName, theme, cart, updateQuantity, onBack, onReviewOrder }: Props) => {
    const isDark = theme === 'dark';
    const { toasts, removeToast } = useToast();

    const products = getPalmOilProducts();
    ...
    return (
        <div className={`min-h-screen pb-40 animate-in fade-in slide-in-from-right-5 duration-500 ${isDark ? 'text-white' : 'text-slate-900'}`}>
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-10">
                <div>
                    <button onClick={onBack} className="flex items-center gap-2 text-sm font-black text-emerald-500 uppercase tracking-widest mb-4 hover:underline">
                    ...
                    <p className="text-sm font-bold text-slate-500 mt-2 uppercase tracking-widest flex items-center gap-2">
                        CATEGORY: <span className="text-emerald-500 font-black">Palm Oil</span>
                ...
                {products.map((product) => (
                    <div key={product.id}
                        className={`group relative p-8 rounded-[40px] border transition-all duration-500 hover:-translate-y-1
                            ${cart[product.id] > 0
                                ? isDark ? 'bg-emerald-950/30 border-emerald-500/30 shadow-xl shadow-emerald-500/5' : 'bg-emerald-50/80 border-emerald-200 shadow-xl shadow-emerald-100'
                                : isDark ? 'bg-slate-900 border-white/5 hover:border-white/10' : 'bg-white border-slate-100 shadow-xl shadow-slate-200/40 hover:shadow-emerald-500/10 hover:border-emerald-100'}`}
                    >
                        {cart[product.id] > 0 && (
                            <div className="absolute top-8 right-8 animate-in zoom-in-50 duration-300">
                                <span className="px-3 py-1 bg-emerald-500 text-white text-[10px] font-black italic uppercase tracking-widest rounded-full shadow-lg shadow-emerald-500/40">IN CART</span>
                            </div>
                        )}
                        <div className="flex gap-8 items-center">
                            <div className={`w-28 h-28 rounded-[32px] flex items-center justify-center text-4xl transition-transform group-hover:scale-110 group-hover:rotate-3
                                ${isDark ? 'bg-slate-800 border border-white/5' : 'bg-amber-50 border border-amber-100 shadow-inner'}`}>
                                🌴
                            </div>
                            <div className="flex-grow min-w-0 flex flex-col">
                                ...
                                <div className="flex flex-wrap items-center justify-between mt-auto gap-4">
                                    ...
                                    <div className={`flex items-center gap-5 p-2 rounded-3xl border ${isDark ? 'bg-slate-950/50 border-white/5' : 'bg-slate-50/50 border-slate-100 shadow-inner'}`}>
                                        ...
                                        />
                                        <button onClick={() => updateQuantity(product.id, 1)} className="w-10 h-10 rounded-2xl bg-emerald-500 text-white flex items-center justify-center transition-all hover:bg-emerald-600 active:scale-90 shadow-lg shadow-emerald-500/30">
            ...
            {totalItems > 0 && (
                <div className="fixed bottom-4 left-4 right-4 lg:bottom-10 lg:left-[calc(18rem+2.5rem)] lg:right-10 z-[100] animate-in slide-in-from-bottom-10 duration-700">
                    <div className={`max-w-6xl mx-auto rounded-[30px] lg:rounded-[50px] p-4 md:p-6 lg:p-8 border backdrop-blur-3xl shadow-2xl transition-all
                        ${isDark ? 'bg-slate-900/90 border-white/10 shadow-emerald-500/10' : 'bg-white/90 border-slate-200 shadow-slate-300'}`}>

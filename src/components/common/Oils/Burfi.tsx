...
import { getBurfiProducts, saveBurfiProducts, getAllProducts } from '../../../constants/productData';
...
const Burfi = ({
    ...
    const [products, setProducts] = useState<Product[]>(getBurfiProducts());
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-10">
                <div>
                    ...
                    <p className="text-sm font-bold text-slate-500 mt-2 uppercase tracking-widest flex items-center gap-2">
                        CATEGORY: <span className={`text-${primaryColor}-500 font-black`}>Burfi</span>
                    ...
                    <div className={`relative rounded-3xl w-full max-w-md shadow-2xl border p-8 ${isDark ? 'bg-slate-900 border-white/10' : 'bg-white border-slate-100'}`}>
                        ...
                        <div className="space-y-4">
                            <div>
                                ...
                                <input className={inputCls} value={productForm.name} onChange={e => setProductForm(f => ({ ...f, name: e.target.value }))} placeholder="e.g. Peanut Barfi" />
                            </div>

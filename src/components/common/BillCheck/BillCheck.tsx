...
const BillCheck = ({ theme, type, userProfileName }: Props) => {
    ...
    const invoiceHTML = (bill: Bill) => {
        ...
        ${items.map((it, i) => `<tr><td style="${B}text-align:center">${i + 1}</td><td style="${B}font-weight:bold">${it.name.toUpperCase()} ${it.size.toUpperCase()}</td><td style="${B}text-align:center;font-weight:bold">${it.quantity} ${it.id.includes('_box') ? 'BOX' : (it.unit === 'CAN' ? 'CANS' : (it.unit || 'UNIT').toUpperCase())}${it.weight ? `<br><span style="font-size:9px;font-style:italic;font-weight:normal">(${it.weight})</span>` : ''}</td><td style="${B}text-align:right">${it.price.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</td><td style="${B}text-align:center">${it.id.includes('_box') ? 'BOX' : (it.unit || 'PCS').toUpperCase()}</td><td style="${B}text-align:right;font-weight:bold">${(it.price * it.quantity).toLocaleString('en-IN', { minimumFractionDigits: 2 })}</td></tr>`).join('')}
